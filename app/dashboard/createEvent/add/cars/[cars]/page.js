...
const SingleCar = () => {
  ...
  const carName = decodeURIComponent(pathname.split("/").pop().replaceAll("_", " "));
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const filteredCar = data.cars.filter(v => v.title === carName);
      setcarData(filteredCar);

      const filteredcarItems = data.items.filter(item => item.title === carName);
      setcarItems(filteredcarItems);
    };
  ...
  if (carData.length === 0) {
    return <div className="text-[#321E1E] text-center text-2xl m-12">Vintage & Rental Cars not found</div>;
  }
