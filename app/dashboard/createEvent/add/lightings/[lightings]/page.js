...
const SingleLighting = () => {
  const { addItemToCart } = useContext(CartContext);
  const pathname = usePathname();
  const lightingName = decodeURIComponent(pathname.split("/").pop().replaceAll("_", " "));
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const filteredLighting = data.lightings.filter(v => v.title === lightingName);
      setlightingData(filteredLighting);

      const filteredlightingItems = data.items.filter(item => item.title === lightingName);
      setlightingItems(filteredlightingItems);
    };
  ...
  if (lightingData.length === 0) {
    return <div className="text-[#321E1E] text-center text-2xl m-12">Lightings & Fireworks not found</div>;
  }
