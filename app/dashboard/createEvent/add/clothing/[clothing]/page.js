...
const getData = async () => {
  try {
    const res = await fetch("http://localhost:3000/api/clothing", { cache: "no-store" });
    if (!res.ok) {
      return null;
    }
    return res.json();
...
const SingleClothing = () => {
  ...
  const clothingName = decodeURIComponent(pathname.split("/").pop().replaceAll("_", " "));
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const filteredClothing = data.clothing.filter(v => v.title === clothingName);
      setclothingData(filteredClothing);

      const filteredclothingItems = data.items.filter(item => item.title === clothingName);
  ...
  if (clothingData.length === 0) {
    return <div className="text-[#321E1E] text-center text-2xl m-12">Clothing & Fashion Designers not found</div>;
  }
