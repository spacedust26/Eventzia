...
const getData = async () => {
  try {
    const res = await fetch("http://localhost:3000/api/decor", { cache: "no-store" });
    if (!res.ok) {
      return null;
    }
    return res.json();
...
const SingleDecor = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const filteredDecor = data.decor.filter(v => v.title === decorName);
      setDecordata(filteredDecor);

      const filteredDecorItems = data.items.filter(item => item.title === decorName);
      setDecoritems(filteredDecorItems);
    };
