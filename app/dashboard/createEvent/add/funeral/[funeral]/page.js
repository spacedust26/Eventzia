...
const SingleFuneral = () => {
  ...
  const funeralName = decodeURIComponent(pathname.split("/").pop().replaceAll("_", " "));
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const filteredFuneral = data.funeral.filter(v => v.title === funeralName);
      setfuneralData(filteredFuneral);

      const filteredfuneralItems = data.items.filter(item => item.title === funeralName);
      setfuneralItems(filteredfuneralItems);
    };
  ...
  if (funeralData.length === 0) {
    return <div className="text-[#321E1E] text-center text-2xl m-12">Funeral Services not found</div>;
  }
