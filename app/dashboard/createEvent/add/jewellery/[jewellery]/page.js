...
const getData = async () => {
  try {
    const res = await fetch("http://localhost:3000/api/jewellery", { cache: "no-store" });
    if (!res.ok) {
      return null;
    }
    return res.json();
...
const SingleJewllery = () => {
  const { addItemToCart } = useContext(CartContext);
  const pathname = usePathname();
  const jewelleryName = decodeURIComponent(pathname.split("/").pop().replaceAll("_", " "));
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const filteredJewellery = data.jewellery.filter(v => v.title === jewelleryName);
      setjewelleryData(filteredJewellery);

      const filteredjewelleryItems = data.items.filter(item => item.title === jewelleryName);
      setjewelleryItems(filteredjewelleryItems);
    };
  ...
  if (jewelleryData.length === 0) {
    return <div className="text-[#321E1E] text-center text-2xl m-12">Jewellery not found</div>;
  }
