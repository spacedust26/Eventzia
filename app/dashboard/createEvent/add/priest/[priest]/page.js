...
const getData = async () => {
  try {
    const res = await fetch("http://localhost:3000/api/priest", { cache: "no-store" });
    if (!res.ok) {
      return null;
    }
    return res.json();
...
const SinglePriest = () => {
  ...
  const priestName = decodeURIComponent(pathname.split("/").pop().replaceAll("_", " "));

  const [priestData, setpriestData] = useState([]);
  const [priestItem, setpriestItem] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const filteredPriest = data.priest.filter(v => v.title === priestName);
      setpriestData(filteredPriest);

      const filteredpriestItem = data.items.filter(item => item.title === priestName);
      setpriestItem(filteredpriestItem);
    };

    fetchData();
  }, [priestName]);

  if (priestData.length === 0) {
    return <div className="text-[#321E1E] text-center text-2xl m-12">Priests & Clergy not found</div>;
  }
